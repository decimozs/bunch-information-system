Public Class InsuredForm
    Private detailsConfirmationInstance As New DetailsConfirmationComponent("Insured Information")
    ...
    Private Sub BackButton()
        ...
        AddHandler backBtn.Click, Sub(sender As Object, e As EventArgs)
                                      Handlers.OpenWindow(Me, GuardianForm)
    ...
    Private Sub InputFirstName()
        firstName = Tools.CreateInputBox(Me, "First name", 195, 40, 421, 320)
    End Sub
    Private Sub InputLastName()
        lastname = Tools.CreateInputBox(Me, "Last name", 195, 40, 670, 320)
    End Sub
    Private Sub InputMiddleInitial()
        middleInitial = Tools.CreateInputBox(Me, "M.I", 103, 40, 919, 320)
    End Sub
    Private Sub InputSSINumber()
        ssiNumber = Tools.CreateInputBox(Me, "SSI No.", 600, 52, 421, 415)
    End Sub
    Private Sub InputMonth()
        month = Tools.CreateInputBox(Me, "Month", 193, 40, 421, 510)
        month.ReadOnly = True
    End Sub
    Private Sub InputDay()
        day = Tools.CreateInputBox(Me, "Day", 128, 40, 671, 510)
    End Sub
    Private Sub InputYear()
        year = Tools.CreateInputBox(Me, "Year", 169, 40, 853, 510)
    End Sub
    Private Sub InputHomeNo()
        homeNo = Tools.CreateInputBox(Me, "Home Number", 276, 40, 421, 607)
    End Sub
    Private Sub InputCellNo()
        cellNo = Tools.CreateInputBox(Me, "Phone Number", 268, 40, 754, 607)
    End Sub
    Private Sub InputEmail()
        email = Tools.CreateInputBox(Me, "Email Address", 336, 40, 421, 704)
    End Sub
    Private Sub InputRelationshipToPatient()
        relationshipToPatient = Tools.CreateInputBox(Me, "Status", 215, 40, 808, 704)
        relationshipToPatient.ReadOnly = True
    End Sub
    Private Sub InputStreetAddress()
        streetAddress = Tools.CreateInputBox(Me, "Street Address", 597, 40, 421, 801)
    End Sub
    Private Sub InputBarangay()
        barangay = Tools.CreateInputBox(Me, "Barangay", 207, 40, 421, 868)
    End Sub
    Private Sub InputCity()
        city = Tools.CreateInputBox(Me, "City", 127, 40, 681, 868)
    End Sub
    Private Sub InputZipCode()
        zipCode = Tools.CreateInputBox(Me, "Zip Code", 160, 40, 858, 868)
    End Sub
    Private Sub MonthDropDownButton()
        monthBoxPanel = Tools.CreatePanel(Me, 231, 312, 404, 554)
        Dim monthBoxInstance As New MonthBox("Insured Information")
        monthBoxPanel.Controls.Add(monthBoxInstance)
        Dim dropDownBtn As New DropDown(monthBoxPanel) With {
            .BackColor = Color.Transparent,
            .Size = New Size(25, 11),
            .Location = New Point(593, 516)
        }
        Me.Controls.Add(dropDownBtn)
    End Sub
    Private Sub RelationshipToPatinetDropDownButton()
        relationshipToPatientPanel = Tools.CreatePanel(Me, 244, 176, 794, 750)
        Dim relationshipStatusToPatientInstance As New RelationshipToPatientBox("Insured Information")
        ...
        Dim dropDownBtn As New DropDown(relationshipToPatientPanel) With {
            .BackColor = Color.Transparent,
            .Size = New Size(25, 11),
            .Location = New Point(992, 711)
        }
        Me.Controls.Add(dropDownBtn)
    End Sub
    Public Sub InsertInsuredData()
        ...
        Dim query As String = "insert into insureds (
            ...
            Using connection = Database.GetConnection
                ...
                Using command = Database.GetCommand(query)
                    Try
                        ...
                        command.Parameters.AddWithValue("@insuredname", Handlers.AutoCapitalizeFirstLetter($"{firstName.Text} {middleInitial.Text}. {lastname.Text}"))
                    ...
                    Catch ex As Exception
                        MsgBox($"Failed to insert insured: {ex}")
                    End Try
                End Using
            End Using
        Catch ex As Exception
    ...
    Private Function ValidateInput() As Boolean
        If String.IsNullOrWhiteSpace(firstName.Text) OrElse
           ...
           String.IsNullOrWhiteSpace(ssiNumber.Text) OrElse
        ...
        If IsPlaceholderEmpty(firstName, "First name") OrElse
           ...
           IsPlaceholderEmpty(ssiNumber, "SSI No.") OrElse
